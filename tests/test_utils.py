...
class TestBirthday:
    ...
    hash_vectors: list[tuple[int, float, int]] = [
        ...
        (384, 0.25, 4800000000000000092684464663841168841538056056222209015808),
        (384, 0.5, 7400000000000000113851121046168386415163203252498351521792),
        (384, 0.75, 9999999999999999438119489974413630815797154428513196965888),
        (
            512,
            1e-17,
    ...
    def test_wp_data(self) -> None:
        for bits, p, n in self.hash_vectors:
            if p < 3:  # We need a differnt test in these cases
                continue
            c = 2**bits
            my_p = utils.pbirthday(n, c)
            assert math.isclose(p, my_p)

            my_n = utils.qbirthday(p, c)
